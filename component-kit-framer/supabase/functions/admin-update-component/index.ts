...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "update") {
      ...
      // section (Part/Panel/Page) is a separate concept from tier/category above — admin-only,
      // never derived by sync, so it doesn't touch tier_manually_set.
      if ("section" in body) {
        if (body.section !== null && !["part", "panel", "page"].includes(body.section)) {
          throw new Error("Invalid section")
        }
        fields.section = body.section
      }
